...
test('should be able to insert a node', () => {
    expect(() => linkedList.insertNode(1, null, null)).toThrow(
        Errors.LINKED_LIST_EMPTY
    );
    ...
    expect(() => linkedList.insertNode(1, linkedList.getNode(2), null)).toThrow(
        Errors.INVALID_NODE_POSITION
    );

    length = linkedList.length;
    const insertedNodeAtBeforePostion = linkedList.insertNode(
        6,
    ...
    length = linkedList.length;
    const insertedNodeAtAfterPostion = linkedList.insertNode(
        12,
